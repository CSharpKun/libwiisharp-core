...
public class BNS
{
    ...
    */
    ...
    private byte[] Encode(byte[] inputFrames)
    {
        ...
        Array.Resize<byte>(
            ref inputFrames,
            inputFrames.Length + (14 - num1) * (bnsInfo.ChannelCount == 2 ? 4 : 2)
        );
    ...
    private byte[] DecodeAdpcm(int channel, int dataOffset)
    {
        ...
        int num6 =
            channel == 0
                ? bnsInfo.Coefficients1[num1 * 2 + 1]
                : bnsInfo.Coefficients2[num1 * 2 + 1];
...
{
    private readonly byte[] magic = new byte[4] { 68, 65, 84, 65 };
...
{
    //Private Variables
    private readonly byte[] magic = new byte[4] { 66, 78, 83, 32 };
...
{
    //Private Variables
    private readonly byte[] magic = new byte[4] { 73, 78, 70, 79 };
