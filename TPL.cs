...
public sealed class TPL : IDisposable
{
    ...
    {
        TPL tpl = new();
        MemoryStream memoryStream = new(File.ReadAllBytes(pathToTpl));
        try
        {
    ...
    {
        TPL tpl = new();
        MemoryStream memoryStream = new(tplFile);
        try
        {
            tpl.ParseTpl(memoryStream);
        }
    ...
    {
        return FromImages([img], [tplFormat], [paletteFormat]);
    }
    ...
    )
    {
        ...
        TPL tpl = new();
        tpl.PrivCreateFromImages([.. imageList], tplFormats, paletteFormats);
        return tpl;
    }
    ...
    {
        PrivCreateFromImages([img], [tplFormat], [paletteFormat]);
    }
...
public class TPL_TextureHeader
{
    ...
    public uint MinFilter { get; set; } = 1;

    public uint MagFilter { get; set; } = 1;

    public uint LodBias { get; set; }
