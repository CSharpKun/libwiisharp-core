...
public class Headers
{
    ...
    public static Headers.HeaderType DetectHeader(byte[] file)
    {
        ...
        return file.Length > 4 && (int)Shared.Swap(BitConverter.ToUInt32(file, 0)) == (int)imd5Magic
            ? HeaderType.IMD5
            : HeaderType.None;
    }
    ...
    public class IMET
    {
        ...
        /// </summary>
        public bool IsShortIMET { get; set; }
        ...
        /// </summary>
        public bool HashesMatch { get; private set; } = true;
        ...
        {
            HeaderType headerType = DetectHeader(fileOrHeader);

            if (headerType is not HeaderType.ShortIMET and not HeaderType.IMET)
                throw new Exception("No IMET Header found!");

            Headers.IMET imet = new();
            if (headerType == HeaderType.ShortIMET)
            {
                imet.IsShortIMET = true;
            }

            using var memoryStream = new MemoryStream(fileOrHeader);
