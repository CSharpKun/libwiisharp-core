...
/// </summary>
public class IosPatcher(ILogger<IosPatcher>? logger = null)
{
    private readonly ILogger<IosPatcher> _logger = logger ?? NullLogger<IosPatcher>.Instance;
    ...
    private int PrivPatchFakeSigning(ref byte[] esModule)
    {
        if (_logger.IsEnabled(LogLevel.Debug))
            _logger.LogDebug("Patching Fakesigning...");
        int num = 0;
        ...
        if (_logger.IsEnabled(LogLevel.Debug))
            if (_logger.IsEnabled(LogLevel.Debug))
                _logger.LogDebug(
                    "Patching Fakesigning Finished... (Patches applied: {PatchesCount})",
                    num
                );
        return num;
    }
    ...
    {
        if (_logger.IsEnabled(LogLevel.Debug))
            _logger.LogDebug("Patching ES_Identify...");
        int num = 0;
    ...
    {
        if (_logger.IsEnabled(LogLevel.Debug))
            _logger.LogDebug("Patching NAND Permissions...");
        int num = 0;
        ...
        if (_logger.IsEnabled(LogLevel.Debug))
            _logger.LogDebug(
                "Patching NAND Permissions Finished... (Patches applied: {PatchesCount})",
                num
            );
        return num;
    }
    ...
    private int PrivPatchVP(ref byte[] esModule)
    {
        ...
        for (int firstIndex = 0; firstIndex < esModule.Length - 4; ++firstIndex)
        {
            if (Shared.CompareByteArrays(esModule, firstIndex, second, 0, 4))
            {
                if (_logger.IsEnabled(LogLevel.Debug))
                    _logger.LogDebug(
                        "Patching for VP at Offset: 0x{Offset}",
                        firstIndex.ToString("x8").ToUpper()
                    );
        ...
        if (_logger.IsEnabled(LogLevel.Debug))
            _logger.LogDebug("Patching VP Finished... (Patches applied: {PatchesCount})", num);
        return num;
    }
    ...
    {
        if (_logger.IsEnabled(LogLevel.Debug))
            _logger.LogDebug("Patching Fakesigning, ES_Identify, NAND Permissions and VP ...");
        int num = 0;
        byte[] second1 = [32, 7, 35, 162];
        byte[] second2 = [32, 7, 75, 11];
        ...
        for (int firstIndex = 0; firstIndex < esModule.Length - 6; ++firstIndex)
        {
            ...
            )
            {
                if (_logger.IsEnabled(LogLevel.Debug))
                    _logger.LogDebug(
                        "Patching Fakesigning at Offset: 0x{Offset}",
                        firstIndex.ToString("x8").ToUpper()
                    );
                esModule[firstIndex + 1] = 0;
                firstIndex += 4;
            ...
            else if (Shared.CompareByteArrays(esModule, firstIndex, second3, 0, 4))
            {
                if (_logger.IsEnabled(LogLevel.Debug))
                    _logger.LogDebug(
                        "Patching ES_Identify at Offset: 0x{Offset}",
                        firstIndex.ToString("x8").ToUpper()
                    );
                esModule[firstIndex + 2] = 0;
                esModule[firstIndex + 3] = 0;
        ...
        if (_logger.IsEnabled(LogLevel.Debug))
            _logger.LogDebug(
                "Patching Fakesigning, ES_Identify, NAND Permissions and VP Finished... (Patches applied: {PatchesCount})",
                num
            );
        return num;
    }
    ...
    {
        if (_logger.IsEnabled(LogLevel.Debug))
            _logger.LogDebug("Scanning for ES Module...");
        string str = "$IOSVersion:";
        for (int index1 = wadFile.NumOfContents - 1; index1 >= 0; --index1)
        {
            if (_logger.IsEnabled(LogLevel.Debug))
                _logger.LogDebug(
                    "Scanning Content #{CurrentContent} of {AllContents}...",
                    index1 + 1,
                    wadFile.NumOfContents
                );
            ...
            for (int index2 = 0; index2 < wadFile.Contents[index1].Length - 64; ++index2)
            {
                if (Encoding.ASCII.GetString(wadFile.Contents[index1], index2, 12) == str)
                {
                    ...
                    {
                        if (_logger.IsEnabled(LogLevel.Debug))
                            _logger.LogDebug(" -> ES Module found!");
                        if (_logger.IsEnabled(LogLevel.Debug))
                            _logger.LogDebug("Scanning for ES Module Finished...");
                        esIndex = index1;
        ...
        if (_logger.IsEnabled(LogLevel.Debug))
            _logger.LogDebug("/!\\/!\\/!\\ ES Module wasn't found! /!\\/!\\/!\\");
