...
public class WAD : IDisposable
{
    ...
    {
        get => HasBanner && BannerApp.Header is Headers.IMET imet ? imet.AllTitles : [];
        set => ChangeChannelTitles(value);
    }

    public bool KeepOriginalFooter { get; set; }
    ...
    {
        if (!HasBanner || BannerApp.Header is not Headers.IMET imet)
            return;
        imet.ChangeTitles(newTitles);
    }
